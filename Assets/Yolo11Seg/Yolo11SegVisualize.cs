    ...
    internal sealed class Yolo11SegVisualize : IDisposable
    {
            ...
            NativeArray<Yolo11Seg.Detection>.ReadOnly detections)
        {
            ...
            // Prepare mask data
            {
                ...
                float maskMin = float.PositiveInfinity;
                float maskMax = float.NegativeInfinity;
                float maskSum = 0f;

                for (int i = 0; i < count * MASK_SIZE; i++)
                {
                    float v = maskData[i];

                    maskMin = Mathf.Min(maskMin, v);
                    maskMax = Mathf.Max(maskMax, v);
                    maskSum += v;
                }

                Debug.Log(
                    $"MASK COEFFICIENTS: " +
                    $"count={count * MASK_SIZE}, " +
                    $"min={maskMin:F4}, " +
                    $"max={maskMax:F4}, " +
                    $"mean={maskSum / (count * MASK_SIZE):F4}"
                );
            }
            ...
            float protoMin = float.PositiveInfinity;
            float protoMax = float.NegativeInfinity;
            float protoSum = 0f;
            ...
            for (int i = 0; i < output1.Length; i++)
            {
                ...
                protoMin = Mathf.Min(protoMin, v);
                protoMax = Mathf.Max(protoMax, v);
                protoSum += v;
            }

            Debug.Log(
                $"PROTO: " +
                $"count={output1.Length}, " +
                $"min={protoMin:F4}, " +
                $"max={protoMax:F4}, " +
                $"mean={protoSum / output1.Length:F4}"
            );
