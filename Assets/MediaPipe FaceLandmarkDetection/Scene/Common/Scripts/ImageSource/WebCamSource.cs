// Copyright (c) 2021 homuler
//
// Use of this source code is governed by an MIT-style
// license that can be found in the LICENSE file or at
// https://opensource.org/licenses/MIT.

using System;
using System.Collections;
using System.Collections.Generic;
using System.Linq;
using System.Runtime.InteropServices;
using UnityEngine;

#if UNITY_ANDROID
using UnityEngine.Android;
using Debug = UnityEngine.Debug;
#endif

namespace Mediapipe.Unity
{
    /// <summary>
    /// MediaPipe webcam image source with Android/Vulkan bridge registration.
    ///
    /// After Unity receives the first real WebCamTexture frame, the native Vulkan
    /// texture pointer is passed to VulkanMediaPipeBridge and the initial slot-0 copy
    /// event is submitted. Subsequent frame copies are managed by
    /// VulkanMediaPipeBridgeManager.
    /// </summary>
    public class WebCamSource : ImageSource
    {
#if UNITY_ANDROID && !UNITY_EDITOR
        private const string NativeLibrary = "VulkanBridge";
        private const int InitialBridgeCopyEvent = 10;

        [DllImport(NativeLibrary)]
        private static extern void SetUnityTexture(IntPtr nativeTexture);

        [DllImport(NativeLibrary)]
        private static extern IntPtr GetRenderEventFunc();
#endif

        private const string _TAG = nameof(WebCamSource);
        private const int TextureStartTimeoutFrames = 2000;
        private const int FirstFrameTimeoutFrames = 120;

        private readonly int _preferableDefaultWidth;
        private readonly ResolutionStruct[] _defaultAvailableResolutions;

        private static readonly object _PermissionLock = new object();
        private static bool _IsPermitted;

        private WebCamTexture _webCamTexture;
        private WebCamDevice? _webCamDevice;
        private WebCamDevice[] _availableSources;

        public WebCamSource(int preferableDefaultWidth, ResolutionStruct[] defaultAvailableResolutions)
        {
            _preferableDefaultWidth = preferableDefaultWidth;
            _defaultAvailableResolutions = defaultAvailableResolutions;
        }

        private WebCamTexture webCamTexture
        {
            get => _webCamTexture;
            set
            {
                _webCamTexture?.Stop();
                _webCamTexture = value;
            }
        }

        private WebCamDevice? webCamDevice
        {
            get => _webCamDevice;
            set
            {
                if (_webCamDevice is WebCamDevice current)
                {
                    if (value is WebCamDevice next && next.name == current.name)
                        return;
                }
                else if (value == null)
                {
                    return;
                }

                _webCamDevice = value;
                resolution = GetDefaultResolution();
            }
        }

        private WebCamDevice[] availableSources
        {
            get
            {
                if (_availableSources == null)
                    _availableSources = WebCamTexture.devices;

                return _availableSources;
            }
            set => _availableSources = value;
        }

        public override int textureWidth => !isPrepared ? 0 : webCamTexture.width;
        public override int textureHeight => !isPrepared ? 0 : webCamTexture.height;
        public override bool isVerticallyFlipped => isPrepared && webCamTexture.videoVerticallyMirrored;
        public override bool isFrontFacing =>
            isPrepared &&
            webCamDevice is WebCamDevice device &&
            device.isFrontFacing;
        public override RotationAngle rotation =>
            !isPrepared ? RotationAngle.Rotation0 : (RotationAngle)webCamTexture.videoRotationAngle;
        public override string sourceName =>
            webCamDevice is WebCamDevice device ? device.name : null;
        public override string[] sourceCandidateNames =>
            availableSources?.Select(device => device.name).ToArray();

#pragma warning disable IDE0025
        public override ResolutionStruct[] availableResolutions
        {
            get
            {
#if (UNITY_ANDROID || UNITY_IOS) && !UNITY_EDITOR
                if (webCamDevice is WebCamDevice device)
                    return device.availableResolutions
                        .Select(resolution => new ResolutionStruct(resolution))
                        .ToArray();
#endif
                return webCamDevice == null ? null : _defaultAvailableResolutions;
            }
        }
#pragma warning restore IDE0025

        public override bool isPrepared => webCamTexture != null;
        public override bool isPlaying => webCamTexture != null && webCamTexture.isPlaying;

        /// <summary>
        /// Requests camera permission and chooses the front-facing camera when present.
        /// </summary>
        private IEnumerator Initialize()
        {
            yield return GetPermission();

            if (!_IsPermitted || webCamDevice != null)
                yield break;

            availableSources = WebCamTexture.devices;

            if (availableSources == null || availableSources.Length == 0)
                yield break;

            WebCamDevice selectedDevice = availableSources[0];

            for (int i = 0; i < availableSources.Length; ++i)
            {
                if (availableSources[i].isFrontFacing)
                {
                    selectedDevice = availableSources[i];
                    break;
                }
            }

            webCamDevice = selectedDevice;
        }

        private IEnumerator GetPermission()
        {
            lock (_PermissionLock)
            {
                if (_IsPermitted)
                    yield break;

#if UNITY_ANDROID
                if (!Permission.HasUserAuthorizedPermission(Permission.Camera))
                {
                    Permission.RequestUserPermission(Permission.Camera);
                    yield return new WaitForSeconds(0.1f);
                }
#elif UNITY_IOS
                if (!Application.HasUserAuthorization(UserAuthorization.WebCam))
                    yield return Application.RequestUserAuthorization(UserAuthorization.WebCam);
#endif

#if UNITY_ANDROID
                if (!Permission.HasUserAuthorizedPermission(Permission.Camera))
                {
                    Debug.LogWarning("Not permitted to use Camera");
                    yield break;
                }
#elif UNITY_IOS
                if (!Application.HasUserAuthorization(UserAuthorization.WebCam))
                {
                    Debug.LogWarning("Not permitted to use WebCam");
                    yield break;
                }
#endif

                _IsPermitted = true;
                yield return new WaitForEndOfFrame();
            }
        }

        public override void SelectSource(int sourceId)
        {
            if (sourceId < 0 || sourceId >= availableSources.Length)
                throw new ArgumentException($"Invalid source ID: {sourceId}");

            webCamDevice = availableSources[sourceId];
        }

        public override IEnumerator Play()
        {
            yield return Initialize();

            if (!_IsPermitted)
                throw new InvalidOperationException("Not permitted to access cameras");

            InitializeWebCamTexture();
            webCamTexture.Play();
            yield return WaitForWebCamTexture();
        }

        public override IEnumerator Resume()
        {
            if (!isPrepared)
                throw new InvalidOperationException("WebCamTexture is not prepared yet");

            if (!webCamTexture.isPlaying)
                webCamTexture.Play();

            yield return WaitForWebCamTexture();
        }

        public override void Pause()
        {
            if (isPlaying)
                webCamTexture.Pause();
        }

        public override void Stop()
        {
            if (webCamTexture != null)
                webCamTexture.Stop();

            webCamTexture = null;
        }

        public override Texture GetCurrentTexture() => webCamTexture;

        /// <summary>
        /// Creates the WebCamTexture using the resolution selected by ImageSource.
        /// </summary>
        private void InitializeWebCamTexture()
        {
            Stop();

            if (webCamDevice is WebCamDevice device)
            {
                webCamTexture = new WebCamTexture(
                    device.name,
                    resolution.width,
                    resolution.height,
                    (int)resolution.frameRate);
                return;
            }

            throw new InvalidOperationException(
                "Cannot initialize WebCamTexture because WebCamDevice is not selected");
        }

        /// <summary>
        /// Waits for Unity to create the camera texture and receive a real camera
        /// frame before registering its native Vulkan texture with the bridge.
        /// </summary>
        private IEnumerator WaitForWebCamTexture()
        {
            int count = 0;
            yield return new WaitUntil(
                () => count++ > TextureStartTimeoutFrames || webCamTexture.width > 16);

            if (webCamTexture.width <= 16)
                throw new TimeoutException("Failed to start WebCam");

            int firstFrameWait = 0;
            while (!webCamTexture.didUpdateThisFrame && firstFrameWait < FirstFrameTimeoutFrames)
            {
                ++firstFrameWait;
                yield return null;
            }

            if (!webCamTexture.didUpdateThisFrame)
            {
                Debug.LogWarning(
                    "[WebCamSource] Timed out waiting for didUpdateThisFrame; " +
                    "registering the camera texture anyway.");
            }

            Debug.Log(
                $"[WebCamSource] Camera ready: {webCamTexture.width}x{webCamTexture.height}, " +
                $"rotation={webCamTexture.videoRotationAngle}, " +
                $"mirrored={webCamTexture.videoVerticallyMirrored}.");

            RegisterVulkanCameraTexture();
        }

        /// <summary>
        /// Passes Unity's native WebCamTexture pointer to the Vulkan bridge and
        /// submits the first slot-0 copy required by bridge initialization.
        /// </summary>
        private void RegisterVulkanCameraTexture()
        {
#if UNITY_ANDROID && !UNITY_EDITOR
            IntPtr nativeTexture = webCamTexture.GetNativeTexturePtr();
            if (nativeTexture == IntPtr.Zero)
            {
                Debug.LogError("[WebCamSource] GetNativeTexturePtr returned null.");
                return;
            }

            IntPtr renderEventFunc = GetRenderEventFunc();
            if (renderEventFunc == IntPtr.Zero)
            {
                Debug.LogError("[WebCamSource] Vulkan bridge render event is unavailable.");
                return;
            }

            SetUnityTexture(nativeTexture);
            GL.IssuePluginEvent(renderEventFunc, InitialBridgeCopyEvent);
#endif
        }

        private ResolutionStruct GetDefaultResolution()
        {
            var resolutions = availableResolutions;

            return resolutions == null || resolutions.Length == 0
                ? new ResolutionStruct()
                : resolutions
                    .OrderBy(value => value, new ResolutionStructComparer(_preferableDefaultWidth))
                    .First();
        }

        /// <summary>
        /// Chooses the camera mode whose width is closest to the preferred width.
        /// Ties prefer the smaller height, then the lower frame rate.
        /// </summary>
        private class ResolutionStructComparer : IComparer<ResolutionStruct>
        {
            private readonly int _preferableDefaultWidth;

            public ResolutionStructComparer(int preferableDefaultWidth)
            {
                _preferableDefaultWidth = preferableDefaultWidth;
            }

            public int Compare(ResolutionStruct a, ResolutionStruct b)
            {
                int aDiff = Mathf.Abs(a.width - _preferableDefaultWidth);
                int bDiff = Mathf.Abs(b.width - _preferableDefaultWidth);

                if (aDiff != bDiff)
                    return aDiff - bDiff;

                if (a.height != b.height)
                    return a.height - b.height;

                return (int)(a.frameRate - b.frameRate);
            }
        }
    }
}
